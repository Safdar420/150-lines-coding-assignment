//  my own assignment 
const Name2 : string = "Safdar Ali";
console.log(`Name : ${Name2}`);

const F_Name3 : string = "Sardar Ali";
console.log(`Father_Name : ${F_Name3}`);

let Class : number = 12;
console.log(`Class : ${Class}`);
Class = 13;
console.log(`Class : ${Class}`);

let College : string = "Islamia Govt Science College";
console.log(`College : ${College}`);
College = "D-J Science College";
console.log(`College : ${College}`);

let email_address : string = "sa5763085@gmail.com";
console.log(`Email Adress : ${email_address}`);
email_address = "aa6865674@gmail.com";
console.log(`Email Adress : ${email_address}`);

let Phone_no : number = +923442844068;
console.log(`Phone NO : ${Phone_no}`);
Phone_no = +923408621402   
console.log(`Phone NO : ${Phone_no}`);

// when i update my name in variable using let my name has update

let  Name11 : string = "Safdar";
console.log(`Name : ${Name11}`);
Name11 = "Sardar Safdar";
console.log(`Name : ${Name11}`);
Name11 = "Sardar Safdar Ali";
console.log(`Name : ${Name11}`);
Name11 = "Sardar Safdar Ali Khan";
console.log(`Name : ${Name11}`);

// father name updated list

let Father_Name :string = "Sardar"
console.log(`Father Name : ${Father_Name}`);
Father_Name = "Sardar Ali";
console.log(`Father Name : ${Father_Name}`);
Father_Name = "Sardar Mawaz";
 console.log(`Father Name : ${Father_Name}`);
 Father_Name = "Sardar Mawaz Khan";
console.log(`Father Name : ${Father_Name}`);

// update class 

let Class1 : number = 9;
console.log(`Class : ${Class1}`);
Class1  = 10;
console.log(`Class : ${Class1}`);
Class1  = 11;
console.log(`Class : ${Class1}`);
Class1  = 12;
console.log(`Class : ${Class1}`);

// updated college :

let college1 : string = "Islamia science college";
console.log(`College : ${college1}`);
college1 = "Islamia science college Karachi";
console.log(`College : ${college1}`);
college1 = "Gov Islamia science college Karachi";
console.log(`College : ${college1}`);
college1 = "Government Islamia science college Karachi";
console.log(`College : ${college1}`);

//  Email update

let email_address1 : string = "sa5763085@gmail.com";
console.log(`Email Adress : ${email_address1}`);
email_address1 = "aa6865674@gmail.com";
console.log(`Email Adress : ${email_address1}`);
email_address1 = "aa6865@gmail.com";
console.log(`Email Adress : ${email_address1}`);
email_address1 = "aa65674@gmail.com";
console.log(`Email Adress : ${email_address1}`);

// update phone no 

let Phone_no1 : number = +923442844068;
console.log(`Phone NO : ${Phone_no1}`);
Phone_no1 = +923408621402   
console.log(`Phone NO : ${Phone_no1}`);
Phone_no1 = +923408621403   
console.log(`Phone NO : ${Phone_no1}`);
Phone_no1 = +923408621405   
console.log(`Phone NO : ${Phone_no1}`);

//  another programe 

let Name112 : string = "Safadr Ali";
Name112 = "Safdar ali";
Name112 = "safdar Ali";
Name112 = "SaFdar Ali";
console.log(`Name : ${Name112}`);

const Name111 : string = "Sardar Safdar Ali Khan";
const F_Name111 : string = "Sardar Ali";
let Age : number = 20 ;
let Mann : string = (`Name : ${Name111} , FatherName : ${F_Name111} , age : ${Age}`);
console.log(Mann);

// My computer basic information :

let windows : string = "wlndow10 pro";
console.log(windows);

let laptop : string = "DELL";
console.log(laptop);

let ssd : number = 128;
console.log(ssd);

let generation : string = "4th generation";
console.log(generation);

let processor : string = "intel core i5";
console.log(processor);

let Ram : string = "8Gb";
console.log(Ram);

// Booleans :

let ismarried : boolean = false ;
console.log(ismarried);

let isstudent : boolean = true ;
console.log(isstudent);

let isteacher : boolean = true ;
console.log(isteacher);

let is25yearsold : boolean = false;
console.log(is25yearsold);

let isyourlaptop : boolean = true ;
console.log(isyourlaptop);

let isyourchild : boolean = false ;
console.log(isyourchild);

let isyourwife : boolean = false ;
console.log(isyourwife);

let isyourcar : boolean = true ;
console.log(isyourcar);
